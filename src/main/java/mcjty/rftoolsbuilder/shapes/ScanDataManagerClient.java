...
//            }
    ...
    public void requestExtraDataClient(int id) {
        RFToolsBuilderMessages.sendToServer(CommandHandler.CMD_REQUEST_SHAPE_DATA, TypedMap.builder().put(CommandHandler.PARAM_ID, id));
    }
    ...
    public int getScanDirtyCounterClient(int id) {
        ...
        if (scan.dirtyRequestTimeout <= 0) {
            RFToolsBuilderMessages.sendToServer(CommandHandler.CMD_REQUEST_SCAN_DIRTY, TypedMap.builder().put(CommandHandler.PARAM_ID, id));
